...
Router.map(function(){
  this.route('samplesTablePage', {
    path: '/table/samples',
    template: 'samplesTablePage'
  });
});
...
Template.samplesTablePage.events({
  ...
  'click .starA': function(event, template){
    ...
    Samples.update({_id: this._id}, {$set: {stars: 1}});
  },
  'click .starB': function(event, template){
    event.stopPropagation();
    Samples.update({_id: this._id}, {$set: {stars: 2}});
  },
  'click .starC': function(event, template){
    event.stopPropagation();
    Samples.update({_id: this._id}, {$set: {stars: 3}});
  },
  'click .starD': function(event, template){
    event.stopPropagation();
    Samples.update({_id: this._id}, {$set: {stars: 4}});
  },
  'click .starE': function(event, template){
    event.stopPropagation();
    Samples.update({_id: this._id}, {$set: {stars: 5}});
  },
  'click .checkbox': function(event, template){
    event.stopPropagation();
    if(this.checked){
      Samples.update({_id: this._id}, {$set: {checked: false}});
    }else{
      Samples.update({_id: this._id}, {$set: {checked: true}});
    }
  },
  'click .flag': function(event, template){
    event.stopPropagation();
    if(this.flagged){
      Samples.update({_id: this._id}, {$set: {flagged: false}});
    }else{
      Samples.update({_id: this._id}, {$set: {flagged: true}});
    }
  },
...
Template.samplesTablePage.helpers({
...
Template.samplesTablePage.rendered = function(){
  Template.appLayout.layout();

  // step A:  initialize the table sorting functionality
  $(this.find('#samplesTable')).tablesorter();
